...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
	    ...
	    int syll = 0;	    
		String cha = "aeiouyAEIOUY";
	    char [] che = word.toCharArray();
	    char [] ch = new char [che.length];
	    boolean [] check = new boolean [che.length+1];
	    ...
	    ch[che.length-1] = che[che.length-1];
	    
	    for (int i = 0; i < cha.length(); i++) {	    	
	    	for (int j = 0; j < ch.length; j++) {	    		
	    		if (ch[j] == cha.charAt(i)) { 
	    			check [j+1] = true;	    			  				    			    			    				    				    				    			 
	    		}
	    	}	   
	    }
	    ...
	    for (int j = 1; j < ch.length+1; j++) {   		
    		if ((check[j] == true && check[j-1] == false)) {     			
    			syll++;	    				    			    			    				    				    				    			 
    		}
    	}	   
	    if(che[che.length-1] == 'e' && check[check.length-2] == true) syll = syll +1;
	    if(che[che.length-1] == 'e' && syll > 1 ) syll = syll - 1 ;
	...
	public double getFleschScore()
	{
		...
		int syllFound = this.getNumSyllables();
		int wordsFound = this.getNumWords();
		int sentFound = this.getNumSentences();
		double FleschScore = 206.835 - 1.015*(1.0 * wordsFound/sentFound) - 84.6*(1.0 * syllFound/wordsFound);

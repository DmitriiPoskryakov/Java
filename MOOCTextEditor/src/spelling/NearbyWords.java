...
public class NearbyWords implements SpellingSuggest {
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		List<String> multiple = new LinkedList<String>();  // string multiple times
		...
		while ( (retList.size() < numSuggestions && queue.size()!= 0) && dist < 4) {
			curr = queue.remove();
			multiple = distanceOne(curr, true);	
			for(String s: multiple) {
			   if (!visited.contains(s) && retList.size() < numSuggestions ) {
				   retList.add(s);
				   visited.add(s);
				   queue.add(s);
   ...
   public static void main(String[] args) {
	  ...
	  /* Dictionary d = new DictionaryHashSet();
	   ...
	   w.insertions(s, retList, true );
	   w.deletions(s, retList, true );
	   retList = w.suggestions(x, 4);
